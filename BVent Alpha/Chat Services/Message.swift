...
//  BVent Alpha
//
//  Created by Rayhan Martiza Faluda on 06/11/18.
                    ...
                    if error == nil {
                        ...
                        storageRef.downloadURL(completion: { (url, error) in
                            if error != nil {
                                ...
                                print(imageURL)
                                let values = ["type": "photo", "content": imageURL, "fromID": currentUserID, "toID": toID, "timestamp": message.timestamp, "isRead": false] as [String : Any]
                                Message.uploadMessage(withValues: values, toID: toID, completion: { (status) in
                                    completion(status)
                                })
                            }
