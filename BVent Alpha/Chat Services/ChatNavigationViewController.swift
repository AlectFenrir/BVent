...
class ChatNavigationViewController: UINavigationController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
    ...
    func fetchUsers()  {
        if let id = Auth.auth().currentUser?.uid {
            ...
            User.downloadAllUsers(posterId: posterId!, completion: {(user) in
    ...
    func fetchUserInfo() {
        if let id = Auth.auth().currentUser?.uid {
            User.info(forUserID: id, completion: {[weak weakSelf = self] (user) in
                DispatchQueue.main.async {
                    weakSelf?.nameLabel.text = user.fullname
...
//        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if self.items.count == 0 {
            ...
            cell.nameLabel.text = self.items[indexPath.row].fullname
