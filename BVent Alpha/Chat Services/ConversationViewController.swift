...
//        let rightButton = UIBarButtonItem.init(image: icon!, style: .plain, target: self, action: #selector(ConversationViewController.showContacts))
...
//        }
    ...
    @objc func pushToUserMesssages(notification: NSNotification) {
        if let user = notification.userInfo?["user"] as? User {
            ...
            self.performSegue(withIdentifier: "my chat segue", sender: self)
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "my chat segue" {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        default:
            ...
            cell.nameLabel.text = self.items[indexPath.row].user.fullname
